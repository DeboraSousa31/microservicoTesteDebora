...
import FormComponent from "@/components/FormOrgaoEmpresaComponent.vue";
  ...
  if (currentSituacaoId.value && currentMode.value !== "cadastrar") {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/orgao/${currentSituacaoId.value}`
      );
...
const pageTitle = computed(() => {
  if (currentMode.value === "editar") {
    return "Editar Orgão/Empresa";
  } else if (currentMode.value === "visualizar") {
    return "Visualizar Orgão/Empresa";
  } else {
    return "Cadastrar Orgão/Empresa";
  }
});
...
const cardTitleInternal = computed(() => {
  ...
  if (currentMode.value === "visualizar") return "Detalhes do Orgão/Empresa";
  ...
  if (window.showHostConfirmDialog) {
    try {
      const confirmed = await window.showHostConfirmDialog({
        message: `Tem certeza que deseja remover este Orgão?`,
        header: "Confirmação de Exclusão",
        icon: "pi pi-exclamation-triangle",
        acceptLabel: "Sim, excluir",
        rejectLabel: "Cancelar",
      ...
      if (confirmed) {
        const response = await axios.delete(
          `${API_BASE_URL}/orgao?id=${currentSituacaoId.value}&idUsuario=32&idCredenciadaUsuario=49393`
        );
        if (response.status >= 200 && response.status < 300) {
...
function handleFormSubmission(eventPayload: {
  ...
  if (eventPayload.id) {
    router.push({ name: "VisualizarOrgao", params: { id: eventPayload.id } });
  } else {
    router.push({ name: "PesquisarOrgao" });
  }
}
