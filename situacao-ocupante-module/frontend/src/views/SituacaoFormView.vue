    ...
    <Card class="content-card">
      <template #title>
        <div class="d-flex align-items-center justify-content-end" style="float: right;">
          <router-link :to="{ name: 'PesquisarSituacaoOcupante' }" custom v-slot="{ navigate }">
            <Button type="button" class="info-button mx-1" icon="pi pi-search" @click="navigate" severity="info"
              label="Pesquisar" raised />
          </router-link>
...
const pageTitle = computed(() => {
  if (currentMode.value === 'editar') {
    return 'Editar Situação Ocupante'
  } else if (currentMode.value === 'visualizar') {
    return 'Visualizar Situação Ocupante';
  } else {
    return 'Cadastrar Situação Ocupante'
  }
});
...
const cardTitleInternal = computed(() => {
  ...
  if (currentMode.value === 'visualizar') return 'Detalhes da Situação Ocupante';
...
function triggerEditMode() {
  if (currentSituacaoId.value) {
    router.push({ name: 'EditarSituacaoProprietario', params: { id: currentSituacaoId.value } });
  }
}
      ...
      if (confirmed) {
        ...
        if (response.status >= 200 && response.status < 300) {
          if (window.showHostToast) {
            ...
            router.push({ name: 'PesquisarSituacaoOcupante' });
          }
...
function handleFormSubmission(eventPayload: { id?: number, mode: 'cadastrar' | 'editar' }) {
  if (eventPayload.id) {
    router.push({ name: 'VisualizarSituacaoProprietario', params: { id: eventPayload.id } });
  } else {
    router.push({ name: 'PesquisarSituacaoOcupante' });
  }
}
...
function handleFormCancellation() {
  if (currentMode.value === 'editar' && currentSituacaoId.value) {
    router.push({ name: 'VisualizarSituacaoProprietario', params: { id: currentSituacaoId.value } });
  } else {
    router.push({ name: 'PesquisarSituacaoOcupante' });
  }
}
