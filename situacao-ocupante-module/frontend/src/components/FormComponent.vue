<template>
  <VeeForm :validation-schema="veeValidateSchema" ref="meuFormularioRef" :initialValues="formData" class="review-form"
    ...
    <div class="row">
      ...
      <div class="mb-2 col-md-4 pb-4">
        <VeeField name="credenciadaSituacaoProprietario" v-model="formData.credenciadaSituacaoProprietario"
          v-slot="{ field, errors }">
            <GenericDropdown v-model="formData.credenciadaSituacaoProprietario"
                :fetch-url="`${API_BASE_URL}/situacao-ocupante/credenciadas`" input-id="credFormUnificado"
                label="Credenciada" :disabled="isViewing" optionLabel="nome" :showClear="true" v-bind="field" :invalid="errors.length > 0" />
        </VeeField>
        <ErrorMessage name="credenciadaSituacaoProprietario" class="p-error" />
      </div>
...
type FormData = {
  ...
  nome: string;
  descricao: string;
  credenciadaSituacaoProprietario: Credenciada | null;
...
const formData = ref<FormData>({
  ...
  idUsuario: 32,
  credenciadaSituacaoProprietario: null
})

const situacaoOcupanteZodSchema = z.object({
  nome: z.string({ required_error: 'O Nome é obrigatório.' })
    .min(1, 'O Nome é obrigatório.')
    .min(3, 'O Nome deve ter pelo menos 3 caracteres.'),
  descricao: z.string({ required_error: 'A Descrição é obrigatória.' })
    .min(1, 'A Descrição é obrigatória.'),
  credenciadaSituacaoProprietario: z.object({
    id: z.number(),
    nome: z.string()
  }, {
...
type FormValidationValues = z.infer<typeof situacaoOcupanteZodSchema>;

const veeValidateSchema = toTypedSchema(situacaoOcupanteZodSchema);
...
async function carregarDados(id: number) {
  loading.value = true;
  try {
    const response = await axios.get(`${API_BASE_URL}/situacao-ocupante/${id}`);
    if (response.status >= 200 && response.status < 300) {
      ...
      resetForm({
        values: {
          ...
          credenciadaSituacaoProprietario: formData.value.credenciadaSituacaoProprietario === null
            ? undefined
            : formData.value.credenciadaSituacaoProprietario,
        }
      });
      ...
      if (dadosApi.idCredenciada != null && dadosApi.nomeCredenciada != null) {
        ...
        formData.value.credenciadaSituacaoProprietario = null;
      }
    } else {
...
async function enviarDadosParaAPI() {
  ...
  const url: string = `${API_BASE_URL}/situacao-ocupante`;
  ...
  const dataToSend = {
    ...
    idCredenciadaSituacaoProprietario: formData.value.credenciadaSituacaoProprietario
      ? formData.value.credenciadaSituacaoProprietario.id
      : null,
  };
  ...
  try {
    ...
    if (response && response.status >= 200 && response.status < 300) {
      ...
      if (isCreating.value) {
        formData.value = { id: null, nome: "", descricao: "", credenciadaSituacaoProprietario: null, idCredenciadaUsuario: 49393, idUsuario: 32 };
      }
      if (window.showHostToast) {
        window.showHostToast({ severity: 'success', summary: 'Sucesso', detail: `Situação Ocupante ${isEditing.value ? 'atualizado' : 'cadastrado'}` });
      }
    } else {
      if (window.showHostToast) {
    ...
    if (window.showHostToast) {
      window.showHostToast({ severity: 'error', summary: 'Erro', detail: `Não foi possível ${action} a Situação Ocupante. Tente novamente.` });
    }
