...
public class CredenciadaRepositoryImpl implements CredenciadaRepository, PanacheRepository<Credenciada> {
    ...
    //     String query = "SELECT c.id, c.usuario.nome FROM Credenciada c WHERE c.id = :id and c.usuario.ativo = true";
    

    // return find(query, Parameters.with("id", credenciadaId))
    //         .project(Credenciada.class)
    //         .firstResult();
    // }
