    ...
    <Card class="content-card">
      <template #title>
        <div class="d-flex align-items-center justify-content-end" style="float: right">
          <router-link :to="{ name: 'PesquisarAtualizacao' }" custom v-slot="{ navigate }">
            <Button
              type="button"
...
import FormComponent from '@/components/FormAtualizacaoComponent.vue'
...
const pageTitle = computed(() => {
  if (currentMode.value === 'editar') {
    return 'Editar Atualização'
  } else if (currentMode.value === 'visualizar') {
    return 'Visualizar Atualização'
  } else {
    return 'Cadastrar Atualização'
  }
})
...
const cardTitleInternal = computed(() => {
  ...
  if (currentMode.value === 'visualizar') return 'Detalhes da Atualização'
...
function triggerEditMode() {
  if (currentSituacaoId.value) {
    router.push({ name: 'EditarAtualizacao', params: { id: currentSituacaoId.value } })
  }
}
  ...
  if (window.showHostConfirmDialog) {
    try {
      const confirmed = await window.showHostConfirmDialog({
        message: `Tem certeza que deseja remover esta Atualização?`,
        header: 'Confirmação de Exclusão',
        icon: 'pi pi-exclamation-triangle',
        acceptLabel: 'Sim, excluir',
        rejectLabel: 'Cancelar',
      ...
      if (confirmed) {
        const response = await axios.delete(
          `${API_BASE_URL}/atualizacao?id=${currentSituacaoId.value}&idUsuario=32&idCredenciadaUsuario=49393`,
        )
        if (response.status >= 200 && response.status < 300) {
          if (window.showHostToast) {
            ...
            router.push({ name: 'PesquisarAtualizacao' })
          }
        } else {
          console.warn('Requisição retornou com status não esperado:', response.status)
          if (window.showHostToast) {
...
function handleFormSubmission(eventPayload: { id?: number; mode: 'cadastrar' | 'editar' }) {
  ...
  if (eventPayload.id) {
    router.push({ name: 'VisualizarAtualizacao', params: { id: eventPayload.id } })
  } else {
    router.push({ name: 'PesquisarAtualizacao' })
  }
}
...
function handleFormCancellation() {
  if (currentMode.value === 'editar' && currentSituacaoId.value) {
    router.push({ name: 'VisualizarAtualizacao', params: { id: currentSituacaoId.value } })
  } else {
    router.push({ name: 'PesquisarAtualizacao' })
  }
}
