  ...
  <div class="view-container d-flex flex-column">
    <div class="header cerurb-header">
      <h3 class="header-title">
        <i class="pi pi-search"></i> Pesquisar Status
      </h3>
    </div>
    <Card class="content-card">
      <template #title>
        <router-link
          :to="{ name: 'CadastrarStatus' }"
          custom
          v-slot="{ navigate }"
        >
          <Button
      ...
      <template #content>
        <GenericDataTable
          ...
          :fetch-data-function="fetchStatus"
        >
          <template #search-fields="{ filters }">
            ...
            <div class="mb-2 col-md-3">
              <GenericDropdown
                v-model="filters.categoriaSelecionada"
                :fetch-url="`${API_BASE_URL}/status/categorias`"
                input-id="categoriaPesquisa"
                label="Categoria"
                optionLabel="nome"
                :showClear="true"
              />
          ...
          <template #columns>
            ...
            <Column field="nomeCategoria" sortable header="Categoria"></Column>
            <Column header="Cor">
              <template #body="slotProps">
                <Avatar
                  v-if="slotProps.data.cor"
                  :label="slotProps.data.legenda"
                  :style="{
                    backgroundColor: slotProps.data.cor,
                    color: '#ffffff',
                  }"
                  shape="circle"
                  size="large"
            ...
            <Column header="Cor do Mapa">
              <template #body="slotProps">
                <Avatar
                  
                  :label="slotProps.data.legenda"
                  :style="{
                    backgroundColor: slotProps.data.corMapa,
                    color: '#ffffff',
...
type Status = { id: number; nome: string; descricao: string };
...
const initialFilters = ref({
  ...
  categoriaSelecionada: null as Categoria | null,
});

const genericTableRef = ref<InstanceType<typeof GenericDataTable> | null>(null);

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function fetchStatus(lazyParams: any, filters: any) {
  try {
    const page = lazyParams.first / lazyParams.rows;
    const params: Record<string, unknown> = {
      nome: filters.nome,
      descricao: filters.descricao,
      idCredenciadaStatus: filters.credenciadaSelecionada?.id,
      idCategoria: filters.categoriaSelecionada?.id,
      page: page,
    ...
    }>(`${API_BASE_URL}/status/pesquisar/`, { params });
    console.log(params);
...
async function removerItem(item: Status) {
    ...
    try {
      ...
      if (confirmed) {
        const response = await axios.delete(
          `${API_BASE_URL}/status?id=${item.id}&idUsuario=32&idCredenciadaUsuario=49393`
        );
        ...
        if (response.status >= 200 && response.status < 300) {
          ...
          if (window.showHostToast) {
            window.showHostToast({
              severity: "success",
              summary: "Sucesso",
              detail: "Status removido com sucesso!",
            });
          }
        } else {
          console.warn(
