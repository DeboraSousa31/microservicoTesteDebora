    ...
    <Card class="content-card">
      <template #title>
        <div class="d-flex align-items-center justify-content-end" style="float: right">
          <router-link :to="{ name: 'PesquisarStatus' }" custom v-slot="{ navigate }">
            <Button
              type="button"
...
import FormComponent from '@/components/FormStatusComponent.vue'
...
const cardTitleInternal = computed(() => {
  ...
  if (currentMode.value === 'visualizar') return 'Detalhes da Status'
...
function triggerEditMode() {
  if (currentSituacaoId.value) {
    router.push({ name: 'EditarStatus', params: { id: currentSituacaoId.value } })
  }
}
    ...
    try {
      ...
      if (confirmed) {
        const response = await axios.delete(
          `${API_BASE_URL}/status?id=${currentSituacaoId.value}&idUsuario=32&idCredenciadaUsuario=49393`,
        )
        if (response.status >= 200 && response.status < 300) {
          if (window.showHostToast) {
            ...
            router.push({ name: 'PesquisarStatus' })
          }
        } else {
...
function handleFormSubmission(eventPayload: { id?: number; mode: 'cadastrar' | 'editar' }) {
  ...
  if (eventPayload.id) {
    router.push({ name: 'VisualizarStatus', params: { id: eventPayload.id } })
  } else {
    router.push({ name: 'PesquisarStatus' })
  }
}
...
function handleFormCancellation() {
  if (currentMode.value === 'editar' && currentSituacaoId.value) {
    router.push({ name: 'VisualizarStatus', params: { id: currentSituacaoId.value } })
  } else {
    router.push({ name: 'PesquisarStatus' })
  }
}
