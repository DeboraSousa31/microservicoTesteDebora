  ...
  <div class="view-container d-flex flex-column">
    <div class="header cerurb-header">
      <h3 class="header-title">
        <i class="pi pi-search"></i> Pesquisar Situação do Jurídica
      </h3>
    </div>
    <Card class="content-card">
      <template #title>
        <router-link
          :to="{ name: 'CadastrarSituacaoJuridica' }"
          custom
          v-slot="{ navigate }"
        >
          <Button
        ...
        <div>Pesquisar Situações Jurídicas</div>
        <hr class="mb-5" />
      </template>
      <template #content>
        <GenericDataTable
          ref="genericTableRef"
          title="Situação Juridica"
          :initial-filters="initialFilters"
          :fetch-data-function="fetchSituacaoJuridica"
        >
...
type SituacaoJuridica = { id: number; nome: string; descricao: string };
...
async function fetchSituacaoJuridica(lazyParams: any, filters: any) {
  try {
    const page = lazyParams.first / lazyParams.rows;
    const params: Record<string, unknown> = {
      nome: filters.nome,
      descricao: filters.descricao,
      idCredenciadaSituacaoJuridica: filters.credenciadaSelecionada?.id,
      page: page,
    ...
    const response = await axios.get<{
      content: SituacaoJuridica[];
      totalElements: number;
    }>(`${API_BASE_URL}/situacao-juridica/pesquisar/`, { params });
    ...
    try {
      ...
      if (confirmed) {
        const response = await axios.delete(
          `${API_BASE_URL}/situacao-juridica?id=${item.id}&idUsuario=32&idCredenciadaUsuario=49393`
        );
