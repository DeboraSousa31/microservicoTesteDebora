<template>
  <div class="view-container d-flex flex-column" style="position: relative; bottom: 3rem; min-height: 70vh">
    ...
    <Card class="content-card">
      <template #title>
        <div class="d-flex align-items-center justify-content-end" style="float: right">
          <router-link :to="{ name: 'PesquisarTipoComodo' }" custom v-slot="{ navigate }">
            <Button
              type="button"
...
const API_BASE_URL = `/api/tipo-comodo`;
...
const pageTitle = computed(() => {
  if (currentMode.value === 'editar') {
    return 'Editar Tipo Cômodo'
  } else if (currentMode.value === 'visualizar') {
    return 'Visualizar Tipo Cômodo'
  } else {
  ...
  if (window.showHostConfirmDialog) {
    try {
      const confirmed = await window.showHostConfirmDialog({
        message: `Tem certeza que deseja remover esta Tipo Cômodo?`,
      ...
      if (confirmed) {
        const response = await axios.delete(
          `${API_BASE_URL}/tipo-comodo?id=${currentSituacaoId.value}&idUsuario=454`,
        )
