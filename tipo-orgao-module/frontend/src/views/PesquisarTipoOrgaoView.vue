  ...
  <div class="view-container d-flex flex-column">
    <div class="header cerurb-header">
      <h3 class="header-title">
        <i class="pi pi-search"></i> Pesquisar Tipo do Órgão
      </h3>
    </div>
    <Card class="content-card">
      <template #title>
        <router-link :to="{ name: 'CadastrarTipoOrgao' }" custom v-slot="{ navigate }">
          <Button type="button" class="info-button" icon="pi pi-plus" severity="info" label="Cadastrar" raised
            @click="navigate" />
        </router-link>
        <div>Pesquisar Tipo do Órgão</div>
        <hr class="mb-5" />
      </template>
      <template #content>
        <GenericDataTable ref="genericTableRef" title="Tipo de Órgão" :initial-filters="initialFilters"
          :fetch-data-function="fetchTipoOrgao">
          <template #search-fields="{ filters }">
            ...
            <div class="mb-2 col-md-4">
              <GenericDropdown v-model="filters.credenciadaSelecionada"
                :fetch-url="`${API_BASE_URL}/tipo-orgao/credenciadas`" input-id="credenciadaPesquisa"
          ...
          <template #columns>
            <Column header="Ações" style="width: 8rem;">
              <template #body="slotProps">
                <div class="div-acoes">
                  <router-link :to="{ name: 'VisualizarTipoOrgao', params: { id: slotProps.data.id } }"
                    custom v-slot="{ href }">
...
type TipoOrgao = { id: number; nome: string; descricao: string; };
...
async function fetchTipoOrgao(lazyParams: any, filters: any) {
  try {
    const page = lazyParams.first / lazyParams.rows;
    const params: Record<string, unknown> = {
      nome: filters.nome,
      descricao: filters.descricao,
      idCredenciadaTipoOrgao: filters.credenciadaSelecionada?.id,
      page: page,
    ...
    const response = await axios.get<{ content: TipoOrgao[], totalElements: number }>(
      `${API_BASE_URL}/tipo-orgao/pesquisar/`, { params }
    );
...
async function removerItem(item: TipoOrgao) {
  if (window.showHostConfirmDialog) {
    try {
      ...
      if (confirmed) {
        const response = await axios.delete(`${API_BASE_URL}/tipo-orgao?id=${item.id}&idUsuario=32&idCredenciadaUsuario=49393`);
