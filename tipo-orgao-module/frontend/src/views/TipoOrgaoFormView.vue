    ...
    <Card class="content-card">
      <template #title>
        <div class="d-flex align-items-center justify-content-end" style="float: right;">
          <router-link :to="{ name: 'PesquisarTipoOrgao' }" custom v-slot="{ navigate }">
            <Button type="button" class="info-button mx-1" icon="pi pi-search" @click="navigate" severity="info"
              label="Pesquisar" raised ></Button>
          </router-link>

          <SplitButton v-if="isViewing && currentSituacaoId" label="Opções" icon="pi pi-cog" :model="splitButtonOptions"
            severity="warning" class="p-button-lg p-0" raised />
        </div>
      ...
      <template #content>
        <FormComponent :mode="currentMode" :situacao-id="currentSituacaoId" @form-submitted="handleFormSubmission"
...
const currentMode = computed(() => route.meta.mode as ('cadastrar' | 'editar' | 'visualizar') || 'cadastrar');
...
const pageTitle = computed(() => {
  if (currentMode.value === 'editar') {
    return 'Editar Tipo de Órgão'
  } else if (currentMode.value === 'visualizar') {
    return 'Visualizar Tipo de Órgão';
  } else {
    return 'Cadastrar Tipo de Órgão'
  }
});
...
const cardTitleInternal = computed(() => {
  ...
  if (currentMode.value === 'visualizar') return 'Detalhes do Tipo de Órgão';
...
function triggerEditMode() {
  if (currentSituacaoId.value) {
    router.push({ name: 'EditarTipoOrgao', params: { id: currentSituacaoId.value } });
  }
}
  ...
  if (window.showHostConfirmDialog) {
    try {
      const confirmed = await window.showHostConfirmDialog({
        message: `Tem certeza que deseja remover este Tipo de Órgão?`,
      ...
      if (confirmed) {
        const response = await axios.delete(`${API_BASE_URL}/tipo-otgso?id=${currentSituacaoId.value}&idUsuario=32&idCredenciadaUsuario=49393`);
        if (response.status >= 200 && response.status < 300) {
          if (window.showHostToast) {
            window.showHostToast({ severity: 'success', summary: 'Sucesso', detail: 'Tipo de Órgão removido com sucesso!' });
            router.push({ name: 'PesquisarTipoOrgao' });
          }

        } else {
          console.warn('Requisição retornou com status não esperado:', response.status);
          if (window.showHostToast) {
            window.showHostToast({ severity: 'error', summary: 'Erro', detail: 'Erro ao tentar remover Tipo de Órgão.' });
          }
        }
      } else {
        if (window.showHostToast) {
...
function handleFormSubmission(eventPayload: { id?: number, mode: 'cadastrar' | 'editar' }) {
  if (eventPayload.id) {
    router.push({ name: 'VisualizarTipoOrgao', params: { id: eventPayload.id } });
  } else {
    router.push({ name: 'PesquisarTipoOrgao' });
  }
}
...
function handleFormCancellation() {
  if (currentMode.value === 'editar' && currentSituacaoId.value) {
    router.push({ name: 'VisualizarTipoOrgao', params: { id: currentSituacaoId.value } });
  } else {
    router.push({ name: 'PesquisarTipoOrgao' });
  }
}
