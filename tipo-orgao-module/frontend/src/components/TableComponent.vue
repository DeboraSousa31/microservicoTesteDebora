<template>
  ...
  >
    <div class="row">
      ...
      <div class="mb-2 col-md-4 pb-4">
        <VeeField
          name="credenciadaTipoOrgao"
          v-model="formData.credenciadaTipoOrgao"
        ...
        >
          ...
          <GenericDropdown
            v-model="formData.credenciadaTipoOrgao"
            :fetch-url="`${API_BASE_URL}/tipo-orgao/credenciadas`"
            input-id="credFormUnificado"
            label="Credenciada *"
            optionLabel="nome"
            :showClear="true"
            ...
            :invalid="errors.length > 0"
          />
          <!-- <GenericDropdown v-model="formData.credenciadaTipoOrgao" :disabled="isViewing" v-bind="field"
...
async function confirmarEnvio() {
  ...
  //   message: `Tem certeza que deseja ${acao} esta Tipo de Órgão?`,
...
async function enviarDadosParaAPI() {
  ...
  const dataToSend = {
    id: isEditing.value ? props.tipoOrgaoId : undefined,
    nome: formData.value.nome,
    descricao: formData.value.descricao,
    idCredenciadaUsuario: 53291,
  ...
  try {
    ...
    if (isEditing.value && props.tipoOrgaoId) {
      response = await axios.put(`${url}`, dataToSend)
      idRetornado = props.tipoOrgaoId
    ...
    if (response && response.status >= 200 && response.status < 300) {
      ...
      if (window.showHostToast) {
        window.showHostToast({
          severity: 'success',
          summary: 'Sucesso',
          detail: `Tipo de Órgão ${isEditing.value ? 'atualizado' : 'cadastrado'}`,
        })
      }
    } else {
    ...
    //appToast.showError(`Não foi possível ${action} a Tipo de Órgão. Tente novamente.`)

    if (window.showHostToast) {
      window.showHostToast({
        severity: 'error',
        summary: 'Erro',
        detail: `Não foi possível ${action} a Tipo de Órgão. Tente novamente.`,
      })
    }
...
watch(
  ...
  (newMode, oldMode) => {
    ...
    } else if ((newMode === 'editar' || newMode === 'visualizar') && props.tipoOrgaoId != null) {
      carregarDados(props.tipoOrgaoId)
    }
  },
  { immediate: true },
)
